...
public sealed class ChatChannelService
{
    ...
    /// </summary>
    private static readonly Dictionary<MessageHistoryService.Category, int> ChannelIds = new()
    {
        [MessageHistoryService.Category.Say]         = 1,
        [MessageHistoryService.Category.Party]       = 2,
        [MessageHistoryService.Category.FreeCompany] = 6,
    };
    ...
    public ChatChannelService(MessageHistoryService history, TolkService tolk, IPluginLog log)
    {
        _history = history;
        _tolk    = tolk;
        _log     = log;
    ...
    public unsafe void TrySwitchToBrowsedChannel()
    {
        ...
        if (category == MessageHistoryService.Category.Tell)
        {
        ...
        if (!ChannelIds.TryGetValue(category, out var channelId))
        {
            ...
            if (IsSendableCategory(category))
                _tolk.SpeakInterrupt(AccessibilityStrings.ChannelNotAvailable(
                    AccessibilityStrings.ChatCategoryName(category)));
            return;
        }
    ...
    private static bool IsSendableCategory(MessageHistoryService.Category category) => category
        is MessageHistoryService.Category.Say
        or MessageHistoryService.Category.Shout
        or MessageHistoryService.Category.Party
        or MessageHistoryService.Category.Alliance
        or MessageHistoryService.Category.Tell
        or MessageHistoryService.Category.FreeCompany;
}
